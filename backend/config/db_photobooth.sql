...
CREATE PROCEDURE proses_reschedule_v1(
    IN p_id_booking INT,
    IN p_id_user INT,
    IN p_tanggal_baru DATE,
    IN p_jam_baru TIME
)
BEGIN
    DECLARE v_id_studio INT;
    ...
    -- 1. Ambil ID Studio dari booking tersebut
    SELECT id_studio INTO v_id_studio FROM booking
    WHERE id_booking = p_id_booking AND id_user = p_id_user;

    IF v_id_studio IS NULL THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Data booking tidak ditemukan';
    ELSE
        -- 2. Cek apakah slot baru sudah di-booking orang lain
        SELECT COUNT(*) INTO v_is_booked FROM booking
        WHERE id_studio = v_id_studio
          AND tanggal = p_tanggal_baru
          AND jam = p_jam_baru
          AND status != 'cancelled'
          AND id_booking != p_id_booking;
        ...
        ELSE
            -- 3. Eksekusi Update
            UPDATE booking
            SET tanggal = p_tanggal_baru,
                jam = p_jam_baru,
                status = 'pending' -- Reset status agar divalidasi ulang admin jika perlu
            WHERE id_booking = p_id_booking;

            SELECT 'Reschedule Berhasil' AS message;
        END IF;
    END IF;
END //
...
SHOW PROCEDURE STATUS WHERE Name = 'proses_reschedule_v1';
...
-- Masukkan kembali data dasar agar API tidak error 500
INSERT IGNORE INTO users (nama_lengkap, username, email, nomor_hp, password, status)
VALUES ('Nida Rahma', 'nidarahma', 'nida@email.com', '081234567890', 'password123', 'active');

INSERT IGNORE INTO kategori (id_kategori, nama_kategori, kode_kategori)
VALUES (1, 'Photobooth', 'PB'), (2, 'Self Photo', 'SP'), (3, 'Studio Pro', 'PS');

INSERT IGNORE INTO studio (id_studio, kode_studio, nama_studio, id_kategori, harga, durasi, url_gambar)
VALUES (1, 'PB-01', 'Neon Glow Booth', 1, 50000, 15, '/images/neon-photobooth.jpeg');
...
ALTER TABLE booking
    MODIFY COLUMN status ENUM('pending', 'waiting', 'confirmed', 'cancelled', 'success', 'paid', 'completed') DEFAULT 'pending';
...
-- Reset semua status yang salah jadi pending
UPDATE booking SET status = 'pending';

-- Pastikan sekali lagi struktur tabelnya benar-benar default ke pending
ALTER TABLE booking MODIFY COLUMN status ENUM('pending', 'waiting', 'confirmed', 'cancelled', 'success', 'paid', 'completed') DEFAULT 'pending';


ALTER TABLE booking
    MODIFY COLUMN status ENUM('pending', 'waiting', 'confirmed', 'cancelled', 'success', 'paid', 'completed')
        NOT NULL DEFAULT 'pending';

-- Hapus trigger lama jika ada
DROP TRIGGER IF EXISTS trg_pembayaran_after_update;

-- Buat ulang dengan logika yang benar (Hanya update jika pembayaran sukses)
DELIMITER //
CREATE TRIGGER trg_pembayaran_after_update
    AFTER UPDATE ON pembayaran
    FOR EACH ROW
...
ALTER TABLE booking
      MODIFY COLUMN status ENUM('pending', 'waiting', 'paid', 'confirmed', 'cancelled', 'completed')
NOT NULL DEFAULT 'pending';

-- Reset data lama yang salah
UPDATE booking SET status = 'pending' WHERE status = 'paid';

SELECT id_booking, kode_booking, status, created_at
FROM booking
...
CREATE TRIGGER trg_pembayaran_after_update
    ...
    FOR EACH ROW
BEGIN
    -- Jika pembayaran berhasil dikonfirmasi admin/sistem
    IF NEW.status_pembayaran = 'success' THEN
        UPDATE booking SET status = 'confirmed' WHERE kode_booking = NEW.kode_booking;
        -- Jika baru upload bukti (waiting), status booking bisa ikut berubah ke 'waiting'
    ELSEIF NEW.status_pembayaran = 'waiting' THEN
        UPDATE booking SET status = 'waiting' WHERE kode_booking = NEW.kode_booking;
    END IF;
END //
...
DROP PROCEDURE IF EXISTS buat_booking_v3 //
...
BEGIN
    ...
    ELSE
        ...
        IF v_is_booked > 0 THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Jadwal studio sudah penuh!';
        ELSE
            -- INSERT tanpa menyertakan status agar mengikuti DEFAULT 'pending'
            INSERT INTO booking (kode_booking, id_user, id_studio, tanggal, jam, total_harga)
            VALUES (
                       CONCAT('BK-', DATE_FORMAT(p_tanggal, '%y%m%d'), '-', LPAD(FLOOR(RAND()*999), 3, '0')),
                       v_id_user,
                       v_id_studio,
                       p_tanggal,
                       p_jam,
            ...
            SELECT 'Booking Berhasil! Silahkan lakukan pembayaran.' AS Info;
        END IF;
    END IF;
END //
...
ALTER TABLE booking ADD COLUMN bukti_pembayaran VARCHAR(255) NULL;


-- 1. Tambahkan kolom bukti_pembayaran ke tabel booking
ALTER TABLE booking ADD COLUMN bukti_pembayaran VARCHAR(255) NULL;

-- 2. Pastikan tabel pembayaran juga tersedia (karena kamu melakukan JOIN di model)
...
-- Menghapus procedure buat booking
DROP PROCEDURE IF EXISTS buat_booking_v3;

-- Menghapus procedure reschedule
DROP PROCEDURE IF EXISTS proses_reschedule_v1;

-- Menghapus trigger pembayaran
DROP TRIGGER IF EXISTS trg_pembayaran_after_update;
...
-- Pastikan pembayaran punya 'waiting'
ALTER TABLE pembayaran
    MODIFY COLUMN status_pembayaran
        ENUM('pending', 'waiting', 'success', 'failed') DEFAULT 'pending';

-- Pastikan booking punya 'waiting'
ALTER TABLE booking
    MODIFY COLUMN status
        ENUM('pending', 'waiting', 'confirmed', 'cancelled', 'completed')
        NOT NULL DEFAULT 'pending';

-- Pastikan kolom bukti_pembayaran ada
ALTER TABLE booking
    ADD COLUMN IF NOT EXISTS bukti_pembayaran VARCHAR(255) NULL;


-- Tambahkan opsi status agar tidak error 'Data Truncated'
ALTER TABLE booking
    MODIFY COLUMN status ENUM('pending', 'waiting', 'confirmed', 'cancelled', 'success', 'paid', 'completed')
        NOT NULL DEFAULT 'pending';

ALTER TABLE pembayaran
    MODIFY COLUMN status_pembayaran ENUM('pending', 'waiting', 'success', 'failed')
        DEFAULT 'pending';

-- Pastikan kolom bukti_pembayaran ada di tabel booking
ALTER TABLE booking ADD COLUMN IF NOT EXISTS bukti_pembayaran VARCHAR(255) NULL;

ALTER TABLE booking

    ADD COLUMN bukti_pembayaran VARCHAR(255) NULL AFTER total_harga;


-- Pastikan tabel booking bisa menerima status 'waiting'
ALTER TABLE booking
    MODIFY COLUMN status ENUM('pending', 'waiting', 'paid', 'confirmed', 'cancelled', 'success', 'completed')
        NOT NULL DEFAULT 'pending';

-- Pastikan tabel pembayaran juga siap
ALTER TABLE pembayaran
    MODIFY COLUMN status_pembayaran ENUM('pending', 'waiting', 'success', 'failed')
        DEFAULT 'pending';
...
-- Jalankan ini di MySQL Workbench/Console kamu
-- 1. Bersihkan path agar konsisten dimulai dari /images/
UPDATE studio SET url_gambar = '/images/neon-photobooth.jpeg' WHERE kode_studio = 'PB-01';
UPDATE studio SET url_gambar = '/images/vintage-photobooth.jpeg' WHERE kode_studio = 'PB-02';
UPDATE studio SET url_gambar = '/images/photobooth.jpeg' WHERE kode_studio = 'PB-03';
UPDATE studio SET url_gambar = '/images/idol-photobooth.jpeg' WHERE kode_studio = 'PB-04';
UPDATE studio SET url_gambar = '/images/monocrome-photobooth.jpeg' WHERE kode_studio = 'PB-05';
UPDATE studio SET url_gambar = '/images/glosir-photobooth.jpeg' WHERE kode_studio = 'PB-06';

UPDATE studio SET url_gambar = '/images/minimalist-self.jpeg' WHERE kode_studio = 'SP-01';
UPDATE studio SET url_gambar = '/images/koran-self.jpeg' WHERE kode_studio = 'SP-02';
UPDATE studio SET url_gambar = '/images/train-self.jpeg' WHERE kode_studio = 'SP-03';

UPDATE studio SET url_gambar = '/images/minimalist-studio.jpeg' WHERE kode_studio = 'PS-01';
UPDATE studio SET url_gambar = '/images/fashion-studio.jpeg' WHERE kode_studio = 'PS-02';
UPDATE studio SET url_gambar = '/images/family-studio.jpeg' WHERE kode_studio = 'PS-03';
UPDATE studio SET url_gambar = '/images/prewedding-studio.jpeg' WHERE kode_studio = 'PS-04';
UPDATE studio SET url_gambar = '/images/product-studio.jpeg' WHERE kode_studio = 'PS-05';
...
-- 1. Tambahkan status 'success' agar query UPDATE tidak error (Data Truncated)
ALTER TABLE booking
    MODIFY COLUMN status ENUM('pending', 'waiting', 'paid', 'confirmed', 'cancelled', 'success', 'completed')
        NOT NULL DEFAULT 'pending';

-- 2. Pastikan kolom bukti_pembayaran ada di tabel booking
-- Jika sudah ada, lewati perintah ini
ALTER TABLE booking ADD COLUMN IF NOT EXISTS bukti_pembayaran VARCHAR(255) NULL AFTER total_harga;

-- 3. Pastikan kolom di tabel pembayaran juga sinkron (Jika kamu tetap ingin pakai tabel ini)
ALTER TABLE pembayaran
    MODIFY COLUMN status_pembayaran ENUM('pending', 'waiting', 'success', 'failed') DEFAULT 'pending';
...
CREATE TRIGGER trg_pembayaran_after_update
    ...
    FOR EACH ROW
BEGIN
    -- Jika di tabel pembayaran statusnya success, maka di booking otomatis ikut confirmed
    IF NEW.status_pembayaran = 'success' THEN
        UPDATE booking SET status = 'confirmed' WHERE kode_booking = NEW.kode_booking;
    END IF;
END //
...
-- Bersihkan kolom bukti_pembayaran di tabel booking
UPDATE booking
SET bukti_pembayaran = REPLACE(bukti_pembayaran, '/images/', '')
WHERE bukti_pembayaran LIKE '/images/%';

-- Bersihkan kolom bukti_transfer di tabel pembayaran
UPDATE pembayaran
SET bukti_transfer = REPLACE(bukti_transfer, '/images/', '')
WHERE bukti_transfer LIKE '/images/%';

-- Paksa status jadi success untuk yang sudah upload
UPDATE booking SET status = 'success' WHERE bukti_pembayaran IS NOT NULL;
